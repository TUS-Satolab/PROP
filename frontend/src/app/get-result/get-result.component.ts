...
export class GetResultComponent implements OnInit {
  ...
  showTree() {
    ...
    this.httpClient
      .post(QUERY_URL, formData, { responseType: "text" })
      .subscribe((query) => {
        if (query === "Finished") {
          this.httpClient
            .post(GET_RESULT_URL, formData, { responseType: "text" })
            .subscribe((data) => {
  ...
  onSubmit() {
    ...
    this.httpClient
      ...
      .subscribe((query) => {
        if (query === "Finished") {
          this.httpClient
            .post(GET_RESULT_URL, formData, { responseType: "arraybuffer" })
            .subscribe((data) => {
  ...
  vertical_increase() {
    if (
      ...
      this.tree.display.phylotree.display.fixed_width[0] =
        this.tree.display.phylotree.display.fixed_width[0] + 1;
  ...
  horizontal_increase() {
    if (
      ...
      this.tree.display.phylotree.display.fixed_width[1] =
        this.tree.display.phylotree.display.fixed_width[1] + 1;
      this.tree.display.update();
    }
  }
  horizontal_decrease() {
  ...
  linear() {
    ...
    this.tree.display.phylotree.display.options[
