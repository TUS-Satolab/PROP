...
MAFFT_CONTAINER = os.environ.get("MAFFT_CONTAINER", "prop_backend_mafft")
CLUSTALW_CONTAINER = os.environ.get("CLUSTALW_CONTAINER", "prop_backend_clustalw")
...
def complete_calc(
    ...
    if align_method == "mafft" and (
        len(data_first_block_stripped) > MAFFT_ARRAY_LENGTH
        or count_n > MAFFT_ARRAY_COUNT
    ):
    ...
    elif align_method == "clustalw" and (
        len(data_first_block_stripped) > CLUSTALW_ARRAY_LENGTH
        or count_n > CLUSTALW_ARRAY_COUNT
    ):
        raise Exception("Linecount maximum exceeded")
    elif align_method == "None" and (
        len(data_first_block_stripped) > NO_ALIGNMENT_ARRAY_LENGTH
        or count_n > NO_ALIGNMENT_ARRAY_COUNT
    ):
        raise Exception("Linecount maximum exceeded")

    alignment(out_align, filename, input_type, align_method, align_clw_opt)
    (score, otus) = distance_matrix(
        out_align, matrix_output, gapdel, input_type, model, plusgap_checked
    )
...
def main(args):
    ...
    (score, otus) = distance_matrix(
        out_align, matrix_output, gapdel, input_type, model, plusgap
    )
    ...
    if align == "None":
        ...
        try:
            client.containers.run(
                image=CLUSTALW_CONTAINER,
                command="clustalw -INFILE="
                + RESULT_FOLDER
                + "/"
                + input_file
                + " -OUTFILE="
                ...
                + "/"
                + out_align
                + " -OUTPUT=FASTA -OUTORDER=INPUT -TYPE="
                + d
                + " "
                + align_clw_opt,
                volumes={"prop_docker_volume": {"bind": "/data", "mode": "rw"}},
                remove=True,
            )
        ...
        try:
            client.containers.run(
                image=MAFFT_CONTAINER,
                command="bash -c 'mafft "
                + "files/"
                + input_file
                + " > files/"
                + out_align
...
def distance_matrix(aligned_input, matrix_output, gapdel, input_type, model, plusgap):
    ...
    function_mapping = {"nuc": "calcDiff_nuc", "ami": "calcDiff_ami"}
...
def phylo_tree_score_otus(
    input_file, tree, path=RESULT_FOLDER, out_tree="out_tree.txt"
):
    # 系統樹作成
    score = []
    otus = []
        ...
        if model == "JC":
            for a in range(otu):
                for b in range(a):
                    ...
                    w = (
                        1.0
                        - (seqData[a].count("-") + seqData[b].count("-")) / lgs * 0.5
                    )
                    for i in range(lgs):
        ...
        if model == "K2P":
            for a in range(otu):
                for b in range(a):
                    ...
                    S = S / lgs
                    try:
                        score[a][b] = 0.75 * w * math.log(w) - 0.5 * w * math.log(
                            (S - P) * math.sqrt(S + P - Q)
                        )
                    except:
                        raise Exception(
        ...
        for b in range(a):
            ...
            if plusgap == "not_checked":
                ...
                if model == "P-distance":
                    ...
                    try:
                        score[a][b] = (
                            0 - math.log(1 - 2 * P - Q) / 2 - math.log(1 - 2 * Q) / 4
                        )
                    except:
                        raise Exception(
                            "log(0) in Distance Matrix Calculation. Check Type and Genetic Difference"
...
def makeNj(score, otuName):
    ...
    while len(score) > 2:
        ...
        clade1.branch_length = (
            score[min_i][min_j] + node_dist[min_i] - node_dist[min_j]
        ) / 2.0
        ...
        for k in range(0, len(score)):
            if k != min_i and k != min_j:
                score[min_j][k] = (
                    score[min_i][k] + score[min_j][k] - score[min_i][min_j]
                ) / 2.0
