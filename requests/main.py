...
def upload_file(f_name):
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            filename_with_id = "{0}_{2}.{1}".format(
                *filename.rsplit(".", 1) + [task_id]
            )
...
def align(task_id=None, filename=None):
    ...
    if request.method == "POST":
        ...
        if not filename.endswith((".fasta", ".fa", ".txt")):
            ...
            res = {"task_id": "None", "msg": "ERROR: File format"}
            return res
        ...
        res = {"task_id": task_id, "msg": "Started"}
        return res
    else:
        ...
        if job.get_status() == "finished":
            if result_kind == "complete":
                ...
                if os.path.exists(ZIPPED_FOLDER + "/" + result_zip):
                    ...
                    zipFilesInDir(
                        UPLOAD_FOLDER,
                        ZIPPED_FOLDER + "/" + result_zip,
                        lambda name: result_id in name,
                        result_id,
                    )
                else:
                    zipFilesInDir(
                ...
                if os.path.exists(ZIPPED_FOLDER + "/" + result_zip):
                    ...
                    send = send_file(
                        app.config["ZIPPED_FOLDER"] + "/" + result_zip,
                        as_attachment=True,
                    )
                    return send
            ...
            flash(
                "The job failed. Either it took longer than 30 minutes or the provided file is not correct."
            )
            # return redirect(request.url)
    ...
    if request.method == "POST":
        ...
        if task_id is not None and file is None:
            ...
            try:
                ...
                res = {
                    "msg": "INFO : Task ID was used because Matrix File was found in the database"
                }
            except:
                (filename, task_id) = upload_file("file")
        ...
        job = q.enqueue(
            ...
            job_timeout="3h",
            result_ttl="168h",
            args=(filename, matrix_output, gapdel, input_type, model, plusgap_checked),
        )
        # return render_template('get_completed_results_form.html', msg=task_id)
        if res["msg"] == "":
            res["msg"] = "Started"
...
def complete():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            res = {"task_id": "File format", "msg": "Error"}
            return res
        if not filename.endswith((".fasta", ".fa", ".txt")):
            flash("File format not correct. Choose fasta file")
            # return redirect(request.url)
        ...
        if align_method == "None":
            ...
            if filename == "alignment_" + task_id + ".txt":
                ...
                shutil.copy(
                    ...
                    os.path.join(app.config["UPLOAD_FOLDER"], out_align),
                )
            # try:
